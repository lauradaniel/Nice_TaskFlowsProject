"""
Unified Intent Processing Module
==================================
This module contains all classes and functions needed for the complete 
intent processing pipeline from Step 1 through final mapping generation.

Classes:
    - IntentBuilder: Handles intent clustering and assignment to categories
    - IntentGenerator: Generates intents from conversation transcripts
    - LiaMappingGenerator: Creates Text-Tools Excel mapping files

Functions:
    - categories2dataframe: Converts category file to DataFrame
    - categories_dataframe2text: Converts DataFrame to category text format
    - clean_intents: Cleans and validates intent assignments
"""
...
# ============================================================================
# INTENT BUILDER CLASS (Used in Step 1 and Step 2)
# ============================================================================

class IntentBuilder:
    """
    Handles clustering and assignment of intents to categories.
    
    Used in:
        - Step 1: cluster_reasons() - Groups similar intents
        - Step 2: assign_reasons() - Assigns intents to predefined categories
    """
    
    def __init__(self, bedrock_client, cluster_prompt, assign_prompt, 
                 example_reasons:str=None, example_output:str=None) -> None:
    ...
    def cluster_reasons(self, reasons:List[str], model_id, max_l3=70) -> str:
        """
        Clusters similar reasons/intents together (Step 1).
        
        Args:
            reasons: List of intent strings to cluster
            model_id: Bedrock model ID to use
            max_l3: Maximum number of L3 categories to create
            
        Returns:
            str: Clustered categorization in yaml-like format
        """
        reasons_str = '\n'.join(reasons)
        data = {"reasons_str": reasons_str, "max_l3": max_l3}
        prompt = self.cluster_prompt.format(**data)

        categories, input_tokens, output_tokens = bedrock.simple_prompt(
            self.bedrock_client, prompt, model_id, max_tokens=5000)
    ...
    def assign_reasons(self, categories_filename:str, reasons: List[str], 
                      model_id: str, start_ind=0)-> str:
        """
        Assigns reasons to predefined categories (Step 2).
        
        Args:
            categories_filename: Path to categories file
            reasons: List of freeform reasons/intents
            model_id: Name of Bedrock Model
            start_ind: Start index of reason
            
        Returns:
            str: CSV-formatted assignment results
        """
        cat_lines = []
        ...
        if self.example_reasons is None:
            classified, input_tokens, output_tokens = bedrock.simple_prompt(
                self.bedrock_client, prompt, model_id, max_tokens=5000)
        else:
            classified, input_tokens, output_tokens = bedrock.few_shot(
                self.bedrock_client, prompt, 
                [(self.example_reasons, self.example_output)], 
                model_id, max_tokens=5000)
...
# ============================================================================
# INTENT GENERATOR CLASS (Used in Step 1)
# ============================================================================
...
class IntentGenerator:
    ...
    '''
    def __init__(self, bedrock_client, prompt: str, categories:Optional[str], 
                 num_lines:int, model_id:str, min_words=5, max_words=10,
                 max_workers = 10, max_tokens=256, additional_instructions='') -> None:
...
# ============================================================================
# LIA MAPPING GENERATOR CLASS (Used in later steps)
# ============================================================================
